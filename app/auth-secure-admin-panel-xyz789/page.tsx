...
import { createProxyUrl } from '@/config/api.config';
  ...
  const login = async (redirect: string) => {
    ...
    try {
      ...
      if (res?.ok && access && refresh) {
        const done = storageUser({ access, refresh }, { username: username, is_admin: true });
        // try {
        //   const haveAdmin = (await axiosInter.get(getAllDepositsUrl())).status !== 403;
        //   done = storageUser({ access, refresh }, { username: username, is_admin: haveAdmin });
        // } catch (err) {
            ...
            <form className="mt-8 gap-3" onSubmit={handleSubmit}>
              ...
              <fieldset>
                ...
                <input
                  ...
                  className={styles.inputField}
                  placeholder="Enter your code from Authenticator"
                />
              </fieldset>
